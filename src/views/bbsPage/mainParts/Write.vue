    ...
    <el-card shadow="hover">
      <div class="articlehead">
        ...
        <div style="min-width:max-content;margin-left:10px;">
          <el-button round class="btn" @click="showSelectDialogue(true)"
            >设置标签</el-button
          >
          <el-tooltip
            class="item"
            effect="dark"
            :content="id ? '更新文章' : '发布文章'"
            :placement="id ? 'top' : 'right'"
          >
            <el-button
              type="primary"
              :loading="uploading"
              :disabled="!hasOneChanged"
              :icon="id ? 'el-icon-upload2' : 'el-icon-check'"
              circle
              @click="uploadArticle"
            ></el-button>
          </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    updateHasOneChanged() {
      this.hasOneChanged =
        this.tagChanged || this.titleChanged || this.bodyChanged;
    },
    onUpdateChosenTags(tags) {
      //交给SelectDialogue的时候设置
      this.chosenTags = tags;
      if (tags.length != this.lastUpdateTags.length) {
        //长度不等情况
