<template>
  <div class="CommentBar">
    <div style="padding:0px;" class="head">
      ...
      <div style="width:100%;">
        <div
          style="width:100%; display: flex;padding: 10px; justify-content:space-between;"
          class=""
        >
          <div>
            <el-tag
            ...
            >
              回复
              {{
                username == replyTo.commenter_info.username
                  ? "自己"
                  : replyTo.commenter_info.username
              }}</el-tag
            >
            <el-tag
              size="mini"
              ...
              v-if="!editable"
              class="hoverShadow"
              >{{ rawCreatedTime | makeTime }}</el-tag
            >
          </div>

          <div>
            <el-tag
              v-if="commentId && editable"
              size="mini"
              :type="uploading ? 'info' : ''"
              class="hoverShadow"
              @click="cancelSelectReply()"
              style="margin-right:10px;"
              >{{ "取消评论" }}</el-tag
            >
            <el-tag
              ...
              class="hoverShadow"
              @click="pushComment()"
              >{{ uploading ? "发布中" : "发布评论" }}</el-tag
            >

            <el-tag
              v-if="!editable && !replyable"
              size="mini"
              :type="uploading ? 'info' : ''"
              class="hoverShadow"
              @click="selectReply()"
            ...
            <el-tag
              v-if="
                !editable &&
                  !replyable &&
                  this.$store.getters.userinfo.name == username
              "
              size="mini"
              @click="deletComment()"
              class="hoverShadow"
        ...
        <div :key="index" v-for="(item, index) in childComments">
          <hr color="#EBEEF5" SIZE="1" />
          <el-collapse-transition>
            <CommentBar
        ...
        if (height <= scrollHeight) {
          ...
          textArea.style.height =
            textArea.scrollHeight > 200
              ? "200px"
              : textArea.scrollHeight + "px"; // 拿到最新的高度改变textarea的高度
        }
      });
    },
    deletComment() {
