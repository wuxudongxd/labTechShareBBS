<template>
  <div class="readPage">
    <el-card :body-style="{ padding: '0px' }" shadow="hover">
      <div style="padding: 14px;" class=" bottomShadowBox selectable">
        ...
        <div style="margin-top: 20px;" class="">
          <el-tag
            ...
            >作者 {{ author.name }}</el-tag
          >
          <el-tag type="info" size="mini" class="info_tags hoverShadow"
            >编辑时间 {{ edit_time }}</el-tag
          >
          <el-tag
            size="mini"
            class="info_tags hoverShadow"
    ...
    <HorizonSpace v-if="this.$store.getters.userinfo"></HorizonSpace>
    ...
    <div :key="index" v-for="(item, index) in commentCurrentPageData">
      <HorizonSpace></HorizonSpace>
      <el-collapse-transition>
        <el-card :body-style="{ padding: '0px' }" shadow="hover">
          <CommentBar
            :replyable="
              item.id == commentCurrentreply1 && commentCurrentreply2 == null
            "
            :username="item.commenter_info.username"
            :userId="item.commenter_info.id"
...
export default {
  ...
  methods: {
    ...
    loadComments(page) {
      ReadComments(this.id, page).then(res => {
        console.log(res);
        switch (res.status) {
          case 200:
            ...
            for (var key in this.commentCurrentPageData) {
              ...
              for (var key1 in this.commentCurrentPageData[key].child_comment) {
                var childComment = this.commentCurrentPageData[key]
                  .child_comment[key1];
              ...
              for (var key1 in this.commentCurrentPageData[key].child_comment) {
                ...
                if (childComment.reply2) {
                  childComment.replyTo =
                    commentsIdToComments[childComment.reply2];
                }
              }
            }
      ...
      ReadArticle(this.$route.params.id).then(res => {
        ...
        switch (res.status) {
          case 200: //加载文章内容
            ...
            this.author.name = res.data.results.author_info.name;
            this.author.id = res.data.results.author_info.id;
                ...
                case 200:
                  ...
                  for (var key in tag_info) {
                    if (tagToCat.hasOwnProperty(tag_info[key].id)) {
                      if (
                        !this.catToTags.hasOwnProperty(
                          tagToCat[tag_info[key].id]
                        )
                      ) {
                        this.catToTags[tagToCat[tag_info[key].id]] = {
                          tags: [],
                          name: tagToCat[tag_info[key].id]
                        };
                      }
                      this.catToTags[tagToCat[tag_info[key].id]].tags.push(
                        tag_info[key].name
                      );
                    }
                  }
